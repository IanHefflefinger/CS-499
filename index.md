...
Using Express.js, I redesigned the app to use the "VC" in MVC (the application does not use a database, so no model was needed). It is best procatice to seperate the model, view, and controller and makes it easier to seperate concerns. The new application easily seperates logic from view. The code is also substantially more efficient because it uses ejs for templating so code can be reused without being retyped.
